...
4. アンテナ線の両端をまたぐようにLC共振回路用にコンデンサーを挿し込むとなお良いでしょう。
5. MOSFETのSourceをESP32のGNDにブレッドボード経由で接続します。

## ビルド手順

* auth.h.exampleをauth.hという名前に変更してご自宅のWi-FiのAP名とパスワードを入れてください。1行で十分ですがAP名とパスワードの組を複数指定するとその中から電波状況の良いものが選ばれます。
